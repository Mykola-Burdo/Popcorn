...
//-----------------------------------------------------------------------------------------------------------------------------------------------
ABall::ABall()
   : Ball_State(EBall_State::EBS_Normal), Ball_X_Pos(0.0), Ball_Y_Pos(Start_Ball_Y_Pos), Ball_Speed(0.0), Ball_Direction(0),
...
void ABall::Move(ALevel *level, int platform_x_pos, int platform_width)
{
   ...
   if (next_y_pos > max_y_pos)
   {
      ...
      {
         if(next_y_pos > (double)max_y_pos + (double)AsConfig::Ball_Size * 2.0) // In order for the ball to fly below the floor, we check its max_y_pos below the visible border
            Ball_State = EBall_State::EBS_Lost;
      }
   }
